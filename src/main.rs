...
use sdl2::{event::Event, keyboard::Keycode,pixels::Color,rect::Rect,render::{Canvas,Texture,TextureCreator},
    video::{Window,WindowContext},EventPump};
type Piece=Vec<Vec<u8>>;
type States=Vec<Piece>;
const TETRIS_HEIGHT:usize=40;
const HIGHSCORE_FILE:&'static str="scores.txt";
const LEVEL_TIMES:[u32;10]=[1000,850,700,600,500,40,300,250,221,190];
...
fn print_game_information(tetris:&Tetris) {
    ...
    println!("Number of lines: {}{}",tetris.nb_lines,if new_highest_lines_sent {"[NEW HIGHSCORE]"}else{""});
...
fn main() {
    let sdl_context=sdl2::init().expect("error inicializacion");
    ...
    let mut event_pump=sdl_context.event_pump().expect("error en el event_pump");
    loop {
